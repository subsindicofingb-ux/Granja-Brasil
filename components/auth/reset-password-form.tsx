...
export function ResetPasswordForm() {
  ...
  useEffect(() => {
    ...
    if (state.error?.includes("NEXT_REDIRECT")) {
      markAppSessionTab();
      window.location.assign("/auth/tab-session?next=%2Fapp");
    }
    ...
    <form action={formAction} className="space-y-4">
      {isRedirecting && (
        <div className="rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">
          Senha salva com sucesso. Entrando no sistema...
        </div>
      )}
      ...
      <Button className="w-full" type="submit" disabled={pending || isRedirecting}>
        {pending ? "Salvando..." : isRedirecting ? "Entrando..." : "Salvar nova senha"}
      </Button>
